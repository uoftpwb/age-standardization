...
Running this file straight thorugh from beginning to end should successfully complete all analysis. 
...
# "Data/WHR2023Table.csv" is data obtained directly from the World Happiness Report Appendices & Data (https://worldhappiness.report/ed/2023/#appendices-and-data) as "Data for Table 2.1".
# We also add a column n, representing the number of observations in each country-year, with data copied manually from the Statistical Appendix 1 for Chapter 2.
WHR_data_table <- read_excel("Data/DataForTable2.1WHR2023.xls") %>%  
...
WHR_data_n <- read.csv("Data/WHR2023Table_n.csv") 

WHR_data_across_years <- merge(WHR_data_table, WHR_data_n, by = c("Country.name", "year"), all.x = TRUE) %>%
  mutate(COUNTRY_ISO3 = ifelse(Country.name == "Kosovo", "XKX", countrycode(Country.name, 'country.name', 'iso3c'))) # Somaliland region and Kosovo was not matched with the countrycode(). Countrycode for Kosovo is added manually, but Somaliland region is missing for 2020 to 2022, we ignore it here.
...
```{r}
# "Data/DataForFigure2.1WHR2023.xls" is data obtained directly from the World Happiness Report Appendices & Data.
...
# TODO DATA FOR PALESTINE IS MISSING
# Impute values for Palestine because data is missing from WHR Data, using backward engineering.
# The contribution of a predictor for a country is computed by multiplying the coefficient of the predictor in the regression by the difference in the raw value of that predictor for that particular country and the lowest value in our data. Thus, backward engineering is the opposite of this procedure. 
# For example, "0.292" comes from Palestine's "Explained by Healthy life expectancy" value in Figure 2.1 from WHR Chapter 2. We divide this value by the coefficient of healthy life expectancy in our regression, then add the minimum value of healthy life expectancy out of all of our countries. This allows us to backwards engineer a value for Palestine's healthy life expectancy used by the authors of the WHR. 
# Similarly, "1.164" comes from Palestine's "Explained by Dystopia + residual" value. 
  mutate(
...
merged_data <- WHR_summary %>%
  ...
  merge(WHR_data_fig, by.x = "Country.name", by.y = "Country.name", all = TRUE, suffixes = c("_Replicated", "_WHR"))
...
comparison <- merged_data %>% 
  mutate(Difference.Life.Ladder = WHR_score_Replicated - WHR_score_WHR,
         Difference.Log.GDP.per.capita = Log.GDP.per.capita_Replicated - Log.GDP.per.capita_WHR,
         Difference.Social.support = Social.support_Replicated - Social.support_WHR,
         Difference.Healthy.life.expectancy.at.birth = Healthy.life.expectancy.at.birth_Replicated - Healthy.life.expectancy.at.birth_WHR,
         Difference.Freedom.to.make.life.choices = Freedom.to.make.life.choices_Replicated - Freedom.to.make.life.choices_WHR,
         Difference.Generosity = Generosity_Replicated - Generosity_WHR,
         Difference.Perceptions.of.corruption = Perceptions.of.corruption_Replicated - Perceptions.of.corruption_WHR,
         Difference.Log.GDP.per.capita.cont = Log.GDP.per.capita.cont_Replicated - Log.GDP.per.capita.cont_WHR,
         Difference.Social.support.cont = Social.support.cont_Replicated - Social.support.cont_WHR,
         Difference.Healthy.life.expectancy.at.birth.cont = Healthy.life.expectancy.at.birth.cont_Replicated - Healthy.life.expectancy.at.birth.cont_WHR,
         Difference.Freedom.to.make.life.choices.cont = Freedom.to.make.life.choices.cont_Replicated - Freedom.to.make.life.choices.cont_WHR,
         Difference.Generosity.cont = Generosity.cont_Replicated - Generosity.cont_WHR,
         Difference.Perceptions.of.corruption.cont = Perceptions.of.corruption.cont_Replicated - Perceptions.of.corruption.cont_WHR) %>%
  summarise(across(starts_with("Difference."), list(mean = ~mean(abs(.x), na.rm=TRUE), max = ~max(abs(.x), na.rm=TRUE), country_with_max_diff = ~Country.name[which.max(abs(.x))])))
...
# Information about the WHO's standard population here: https://cdn.who.int/media/docs/default-source/gho-documents/global-health-estimates/gpe_discussion_paper_series_paper31_2001_age_standardization_rates.pdf
# Standard population by WHO includes from age 0. Thus, these weights are re-weighted to be only from Age 15 or above by adding adding together the proportions which are in 5 year intervals from the WHO, to get 15 year age groups starting from age 15. Although the sum of these do not equal 100 anymore, this isn't a concern as weighted means will reweight all weights to equal 1 automatically.
standard_population <- c(24.62, 21.35, 15.96, 8.89, 3.06)
...
# The World Happiness Report is replicated best by using YEAR_CALENDAR to represent years. However, we use YEAR_WAVE.
# YEAR_CALENDAR refers to the calendar year in which the data collection occurred. 
# YEAR_WAVE refers to the year of the wave of data collection. Occasionally, these two variables do not match because some data collection waves are not completed fully within the calendar year. 
# Our justification for using YEAR_WAVE comes from the analytical challenges that potentially stem from categorizing by YEAR_CALENDAR, caused by factors such as changes in survey methods or the fact that the sample weights of Gallup data is based on YEAR_WAVE.
...
age_standardized_ranking <-  gallup_across_years %>% 
  ...
  mutate(AS_score = WeightedMeanLS,
...
WHR_AS_comparison <- merge(WHR_summary, age_standardized_ranking, by="COUNTRY_ISO3", all=FALSE) %>%
  mutate(AS_rank = rank(-AS_score),
         rank_difference =  AS_rank - WHR_rank,
         score_difference = AS_score - WHR_score)
```
  ...
  specific_contributions <- age_group_data %>%
    mutate(
      ...
      Perceptions.of.corruption.cont = (coef(regression_model)["Perceptions.of.corruption"] * Perceptions.of.corruption - min(coef(regression_model)["Perceptions.of.corruption"] * age_group_data$Perceptions.of.corruption, na.rm = TRUE)) ,
         "Dystopia + residual" = AS.LS - (Log.GDP.per.capita.cont + Social.support.cont +  Healthy.life.expectancy.at.birth.cont + Freedom.to.make.life.choices.cont + Generosity.cont + Perceptions.of.corruption.cont)
    )
...
# Initialize list to contain data for regression
...
```{r}
# From loop, we have data1, ..., data5. Stack them to create across_age_regression_data
across_age_regression_data <- bind_rows(data1, data2, data3, data4, data5)%>%
  mutate(age_group = as.factor(age_group))

# Standardize columns 2:7 and add them to across_age_regression_data
stanardized_across_age_regression_data <- across_age_regression_data %>%
  dplyr::select(Log.GDP.per.capita:Perceptions.of.corruption) %>%
  scale() %>%
  as.data.frame() %>%
  setNames(paste0("Standard.", colnames(.))) %>%
  bind_cols(across_age_regression_data, .)

# Fit model with all age groups
across_age_model <- feols(
  ...
  data = stanardized_across_age_regression_data)

# Create data frame for model summary
across_age_model_summary <- summary(across_age_model)$coefficients %>%
  as.data.frame() %>%
  rownames_to_column(var = "RowName") %>%
  filter(!(RowName %in% c("age_group2", "age_group3", "age_group4", "age_group5"))) %>%
...
# GROUP BY variable
# SORT BY group
# MUTATE each of second-fifth element + first element
# IF group == 1
# ELSE variable = first variable + variable

a <- across_age_model_summary$Estimate[1:6]
b <- as.numeric(a[1]) + as.numeric(across_age_model_summary$Estimate[7:10])
c <- as.numeric(a[2]) + as.numeric(across_age_model_summary$Estimate[11:14])
d <- as.numeric(a[3]) + as.numeric(across_age_model_summary$Estimate[15:18])
e <- as.numeric(a[4]) + as.numeric(across_age_model_summary$Estimate[19:22])
f <- as.numeric(a[5]) + as.numeric(across_age_model_summary$Estimate[23:26])
g <- as.numeric(a[6]) + as.numeric(across_age_model_summary$Estimate[27:30])
values <- c(a, b, c, d, e, f, g)
across_age_model_summary$values <- values

AS_coefficients_plot <- ggplot(across_age_model_summary, 
                               aes(fill=variables, y=values, x=group)) + 
  geom_bar(position="stack", stat="identity") +
  theme_bw() +
  scale_x_discrete(labels = c("15-29", "30-44", "45-59", "60-74", "Over 74")) +
  scale_fill_discrete(labels = c("Log GDP per capita", "Social support", 
                                 "Healthy life expectancy at birth", 
                                 "Freedom to make life choices",
                                 "Generosity", "Perceptions of corruption")) +
  labs(fill = "Variables",
       x = "Age Group", 
       y = "Standardized Contribution to Life Satisfaction",  
       title = "Determinants of Age-Specific Life Satisfaction") 
...
ggsave("AS_coefficients_plot.png", plot = AS_coefficients_plot, width = 8, height = 6, units = "in", dpi = 300)
ggsave("AS_coefficients_plot.jpg", plot = AS_coefficients_plot, width = 8, height = 6, units = "in", dpi = 300)
ggsave("AS_coefficients_plot.svg", plot = AS_coefficients_plot, width = 8, height = 6)
```

Hypothesis Tests to compare coefficients for each predictor between each age group

Joint hypothesis tests allows for us to test different hypotheses about our coefficients, rather than the default hypothesis that one individual coefficient is equal to 0. We use the "across_age_model", which is a regression using data from all age groups, to test the hypotheses that a predictor has a statistically significant difference in their relationship to life satisfaction when we compare different age groups. 
```{r}
# Joint hypothesis test for all predictors, comparing age groups 1-2, 2-3, 3-4, 4-5, and 1-5
joint_hypo_12 <- glht(across_age_model, linfct = c("age_group2:Standard.Log.GDP.per.capita +
...
joint_hypo_23 <- glht(across_age_model, linfct = c("age_group3:Standard.Log.GDP.per.capita + 
...
joint_hypo_34 <- glht(across_age_model, linfct = c("age_group4:Standard.Log.GDP.per.capita + 
...
joint_hypo_45 <- glht(across_age_model, linfct = c("age_group5:Standard.Log.GDP.per.capita + 
...
joint_hypo_15 <- glht(across_age_model, linfct = c("age_group5:Standard.Log.GDP.per.capita +
                                              age_group5:Standard.Social.support +
                                              age_group5:Standard.Healthy.life.expectancy.at.birth + 
...
# Joint hypothesis test for GDP predictor, comparing age group 1-2, 2-3, 3-4, 4-5, and 1-5
gdp_hypo_12 <- glht(across_age_model, linfct = c("age_group2:Standard.Log.GDP.per.capita = 0"))
gdp_hypo_23 <- glht(across_age_model, linfct = c("age_group3:Standard.Log.GDP.per.capita -
                                          age_group2:Standard.Log.GDP.per.capita = 0"))
gdp_hypo_34 <- glht(across_age_model, linfct = c("age_group4:Standard.Log.GDP.per.capita -
                                          age_group3:Standard.Log.GDP.per.capita = 0"))
gdp_hypo_45 <- glht(across_age_model, linfct = c("age_group5:Standard.Log.GDP.per.capita -
                                          age_group4:Standard.Log.GDP.per.capita = 0"))
gdp_hypo_15 <- glht(across_age_model, linfct = c("age_group5:Standard.Log.GDP.per.capita = 0"))

# Joint hypothesis test for social support predictor, comparing age group 1-2, 2-3, 3-4, 4-5, and 1-5
ss_hypo_12 <- glht(across_age_model, linfct = c("age_group2:Standard.Social.support = 0"))
ss_hypo_23 <- glht(across_age_model, linfct = c("age_group3:Standard.Social.support - age_group2:Standard.Social.support = 0"))
ss_hypo_34 <- glht(across_age_model, linfct = c("age_group4:Standard.Social.support - age_group3:Standard.Social.support = 0"))
ss_hypo_45 <- glht(across_age_model, linfct = c("age_group5:Standard.Social.support - age_group4:Standard.Social.support = 0"))
ss_hypo_15 <- glht(across_age_model, linfct = c("age_group5:Standard.Social.support = 0"))

# Joint hypothesis test for healthy life expectancy predictor, comparing age group 1-2, 2-3, 3-4, 4-5, and 1-5
le_hypo_12 <- glht(across_age_model, linfct = c("age_group2:Standard.Healthy.life.expectancy.at.birth = 0"))
le_hypo_23 <- glht(across_age_model, linfct = c("age_group3:Standard.Healthy.life.expectancy.at.birth -
                                         age_group2:Standard.Healthy.life.expectancy.at.birth = 0"))
le_hypo_34 <- glht(across_age_model, linfct = c("age_group4:Standard.Healthy.life.expectancy.at.birth -
                                         age_group3:Standard.Healthy.life.expectancy.at.birth = 0"))
le_hypo_45 <- glht(across_age_model, linfct = c("age_group5:Standard.Healthy.life.expectancy.at.birth -
                                         age_group4:Standard.Healthy.life.expectancy.at.birth = 0"))
le_hypo_15 <- glht(across_age_model, linfct = c("age_group5:Standard.Healthy.life.expectancy.at.birth = 0"))

# Joint hypothesis test for freedom to make life choices predictor, comparing age group 1-2, 2-3, 3-4, 4-5, and 1-5
fr_hypo_12<- glht(across_age_model, linfct = c("age_group2:Standard.Freedom.to.make.life.choices = 0"))
fr_hypo_23 <- glht(across_age_model, linfct = c("age_group3:Standard.Freedom.to.make.life.choices -
                                         age_group2:Standard.Freedom.to.make.life.choices = 0"))
fr_hypo_34 <- glht(across_age_model, linfct = c("age_group4:Standard.Freedom.to.make.life.choices -
                                         age_group3:Standard.Freedom.to.make.life.choices = 0"))
fr_hypo_45 <- glht(across_age_model, linfct = c("age_group5:Standard.Freedom.to.make.life.choices -
                                         age_group4:Standard.Freedom.to.make.life.choices = 0"))
fr_hypo_15<- glht(across_age_model, linfct = c("age_group5:Standard.Freedom.to.make.life.choices = 0"))

# Joint hypothesis test for generosity predictor, comparing age group 1-2, 2-3, 3-4, 4-5, and 1-5
ge_hypo_12 <- glht(across_age_model, linfct = c("age_group2:Standard.Generosity = 0"))
ge_hypo_23 <- glht(across_age_model, linfct = c("age_group3:Standard.Generosity - age_group2:Standard.Generosity = 0"))
ge_hypo_34 <- glht(across_age_model, linfct = c("age_group4:Standard.Generosity - age_group3:Standard.Generosity = 0"))
ge_hypo_45 <- glht(across_age_model, linfct = c("age_group5:Standard.Generosity - age_group4:Standard.Generosity = 0"))
ge_hypo_15 <- glht(across_age_model, linfct = c("age_group5:Standard.Generosity = 0"))

# Joint hypothesis test for perception of corruption predictor, comparing age group 1-2, 2-3, 3-4, 4-5, and 1-5
co_hypo_12 <- glht(across_age_model, linfct = c("age_group2:Standard.Perceptions.of.corruption = 0"))
co_hypo_23 <- glht(across_age_model, linfct = c("age_group3:Standard.Perceptions.of.corruption - 
                                         age_group2:Standard.Perceptions.of.corruption = 0"))
co_hypo_34 <- glht(across_age_model, linfct = c("age_group4:Standard.Perceptions.of.corruption - 
                                         age_group3:Standard.Perceptions.of.corruption = 0"))
co_hypo_45 <- glht(across_age_model, linfct = c("age_group5:Standard.Perceptions.of.corruption - 
                                         age_group4:Standard.Perceptions.of.corruption = 0"))
co_hypo_15 <- glht(across_age_model, linfct = c("age_group5:Standard.Perceptions.of.corruption = 0"))
```
...
world_map_plot <- ggplot(world_map_plot, aes(x = long, y = lat, group = group, fill = bin_plot)) +
  geom_polygon(color = "black", size = 0.1) +
  scale_fill_manual(values = c(
    "#67001F", "#B2182B", "#D6604D", "#F4A582", "#FDDBC7",
    "#D1E5F0", "#92C5DE", "#4393C3", "#2166AC", "#053061", "grey50"
  ),
  drop = FALSE,
  ...
  ggtitle("Differences between Simple and Age-standardized Averages of Life Satisfaction") +
  labs(
    x = "",
    y = "",
    fill = "Difference"
...
ggsave("world_map_plot.png", plot = world_map_plot, width = 10, height = 7, bg = "white")
ggsave("world_map_plot.jpg", plot = world_map_plot, width = 10, height = 7, bg = "white")
ggsave("world_map_plot.svg", plot = world_map_plot, width = 10, height = 7)
...
columns_to_select <- c("COUNTRY_ISO3", "AS.LS", "AS_rank")
age_specific_ranking_comparisons <- specific_contributions_list %>% 
   Reduce(function(dtf1, dtf2) {
    left_join(dtf1, dtf2 %>% dplyr::select(all_of(columns_to_select)), by = "COUNTRY_ISO3")
  }, .) %>% 
  dplyr::select(!WHR_score:age_group) %>% 
  rename(
    AG1_score = AS.LS.x,
    AG1_rank = AS_rank.x,
    AG2_score = AS.LS.y,
    AG2_rank = AS_rank.y,
    AG3_score = AS.LS.x.x,
    AG3_rank = AS_rank.x.x,
    AG4_score = AS.LS.y.y,
    AG4_rank = AS_rank.y.y,
    AG5_score = AS.LS,
    AG5_rank = AS_rank
  )
...
Creating Table for Ranking Comparison
```{r}
library(xtable)

ranking_comparisons_table <- ranking_comparisons_summary %>%
  mutate("WHR" = paste(WHR_rank, " (", sprintf("%.3f", round(WHR_score, 3)), ")", sep = ""),
         "Standardized" = paste(AS_rank, " (", sprintf("%.3f", round(AS_score, 3)), ")", sep = ""),
         "15-29 Y.O." = paste(AG1_rank, " (", sprintf("%.3f", round(AG1_score, 3)), ")", sep = ""),
         "30-44 Y.O." = paste(AG2_rank, " (", sprintf("%.3f", round(AG2_score, 3)), ")", sep = ""),
         "45-59 Y.O." = paste(AG3_rank, " (", sprintf("%.3f", round(AG3_score, 3)), ")", sep = ""),
         "60-74 Y.O." = paste(AG4_rank, " (", sprintf("%.3f", round(AG4_score, 3)), ")", sep = ""),
         "Over 74 Y.O." = paste(AG5_rank, " (", sprintf("%.3f", round(AG5_score, 3)), ")", sep = "")) %>% 
  dplyr::select(Country = region, "WHR", "Standardized", "15-29 Y.O.",  "30-44 Y.O.", "45-59 Y.O.", "60-74 Y.O.", "Over 74 Y.O.")

latex_table <- xtable(ranking_comparisons_table)

# Print the LaTeX table
print(latex_table, type = "latex")

# Save the LaTeX table to a .tex file
print(latex_table, type = "latex", file = "my_table.tex")
```

Visualizing Ranking Changes
```{r}
ranking_comparison_wide <- ranking_comparisons %>% 
  dplyr::select(COUNTRY_ISO3, COUNTRYNAME = region, WHR_rank, AS_rank, AG1_rank, AG2_rank, AG3_rank, AG4_rank)
...
WHR_and_AG2 <- ranking_comparison_wide %>% dplyr::select('WHR_rank', 'AG2_rank')
WHR_and_AG3 <- ranking_comparison_wide %>% dplyr::select('WHR_rank', 'AG3_rank')
...
WHR_and_AG2 <- WHR_and_AG2 %>%
  arrange(`AG2_rank`)
WHR_and_AG2$AG2_rank <- WHR_and_AG2$WHR

WHR_and_AG3 <- WHR_and_AG3 %>%
  arrange(`AG3_rank`)
WHR_and_AG3$AG3_rank <- WHR_and_AG3$WHR
...
ranking_comparison_visualization_data  <- data.frame(ranking_comparison_wide %>% dplyr::select(WHR_rank),
                                                      ...
                                                      WHR_and_AG2 %>% dplyr::select(AG2_rank),
                                                      WHR_and_AG3 %>% dplyr::select(AG3_rank),
                                                      WHR_and_AG4 %>% dplyr::select(AG4_rank),
                                                     ranking_comparison_wide %>% dplyr::select(WHR_rank)) %>% 
  dplyr::select("WHR" = WHR_rank, "Age-Standardized" = AS_rank, "Age Group 1" = AG1_rank, "Age Group 2" = AG2_rank, 
         "Age Group 3" = AG3_rank, "Age Group 4" = AG4_rank, "WHR2" = WHR_rank.1)


df_long <- reshape2::melt(ranking_comparison_visualization_data, id.vars = "WHR2")

# Get the number of levels in factor
num_levels <- length(levels(factor(df_long$value)))

# Create a vector of labels
labels <- rep(c("Viridis", "Guatemala", "Serbia"), length.out = num_levels)

# Use this vector in scale_fill_manual() function
# Black is Guatemala, White is Serbia
ranking_comparison_plot <- ggplot(df_long, aes(x = variable, y = WHR2, fill = factor(value))) +
  geom_tile() +
  scale_fill_manual(
    values = c(
  ...
  labs(title = "Comparing Rankings of \nSimple, Age-Standardized, \nand Age-Specific Averages", x = "Ranking Type", y = "Ranking") +
  theme_minimal() +
  scale_y_reverse(breaks = c(1, 50, 100)) + 
  scale_x_discrete(labels = c("Simple Average", "Age-standardized", "Age Specific: 15-29", "Age Specific: 30-44",
                              "Age Specific: 45-59", "Age Specific: 60-74")) + 
  guides(fill = guide_colourbar(reverse = TRUE, title = "Ranking")) +
  theme(axis.text.x = element_text(angle = 70, hjust = 1),
        panel.grid = element_blank())  

ggsave("ranking_comparison_plot.png", plot = ranking_comparison_plot, width = 5, height = 10, bg = "white")
ggsave("ranking_comparison_plot.jpg", plot = ranking_comparison_plot, width = 5, height = 10, bg = "white")
ggsave("ranking_comparison_plot.svg", plot = ranking_comparison_plot, width = 5, height = 10, bg = "white")
...
legend_data <- data.frame(
  country = factor(c("Guatemala", "Serbia")),
  value = c("1", "2")
)

# Create the legend plot
legend_plot <- ggplot(legend_data, aes(x = country, y = value, fill = country)) +
  geom_tile() +
  scale_fill_manual(values = c("Guatemala" = "black", "Serbia" = "white")) +
  theme_void() +
  labs(fill = "Legend") +
...
egypt_n <- age_n_matrix %>% dplyr::select("YEAR_WAVE", "age_group", "EGY") %>% 
  filter((YEAR_WAVE <= 2012 & YEAR_WAVE >= 2010)|(YEAR_WAVE <= 2022 & YEAR_WAVE >= 2020))

egypt <- age_matrix %>% dplyr::select("YEAR_WAVE", "age_group", "EGY") %>% 
  filter((YEAR_WAVE <= 2012 & YEAR_WAVE >= 2010)|(YEAR_WAVE <= 2022 & YEAR_WAVE >= 2020))

egypt = merge(egypt, egypt_n, by=c("YEAR_WAVE", "age_group"))

egypt <- egypt %>%
  mutate(Year_Group = case_when(
    between(YEAR_WAVE, 2010, 2012) ~ "2010-2012",
    between(YEAR_WAVE, 2020, 2022) ~ "2020-2022"
  ))
...
iceland <- age_matrix %>% dplyr::select("YEAR_WAVE", "age_group", "ISL") %>% 
  filter((YEAR_WAVE <= 2012 & YEAR_WAVE >= 2010)|(YEAR_WAVE <= 2022 & YEAR_WAVE >= 2020))

iceland_n <- age_n_matrix %>% dplyr::select("YEAR_WAVE", "age_group", "ISL") %>% 
  filter((YEAR_WAVE <= 2012 & YEAR_WAVE >= 2010)|(YEAR_WAVE <= 2022 & YEAR_WAVE >= 2020))

iceland = merge(iceland, iceland_n, by=c("YEAR_WAVE", "age_group"))
...
Investigate components of age-standardized life satisfaction by finding life satisfaction for each country, age group
```{r}
life_satisfaction <- gallup_means %>%
  filter(YEAR_WAVE %in% 2020:2022) %>%
  group_by(COUNTRY_ISO3, age_group) %>%
  summarise(WeightedMeanLS = weighted.mean(WeightedMeanLS, n, na.rm=T), .groups = "drop") %>% 
  pivot_wider(names_from = COUNTRY_ISO3, values_from = WeightedMeanLS) %>% 
  filter(age_group != "allPopulation") %>%  
  mutate(across(AFG:ZWE, ~ round(., 3)))

prop_obs <- gallup_means %>%
  filter(YEAR_WAVE %in% 2020:2022) %>%
  group_by(COUNTRY_ISO3, age_group) %>%
  summarise(n=sum(n), .groups = "drop") %>% 
  pivot_wider(names_from = COUNTRY_ISO3, values_from = n) %>% 
  filter(age_group != "allPopulation") %>%  
  mutate(across(AFG:ZWE, ~. / sum(.))) %>% 
  mutate(across(AFG:ZWE, ~ round(., 3)))

num_obs <- gallup_means %>%
  filter(YEAR_WAVE %in% 2020:2022) %>%
  group_by(COUNTRY_ISO3, age_group) %>%
  summarise(n=sum(n), .groups = "drop") %>% 
  pivot_wider(names_from = COUNTRY_ISO3, values_from = n) %>% 
...
standard_population_percentages <- round(prop.table(standard_population) * 100, 2)
print(standard_population_percentages)
```

Analyzing General Trends
```{r}
# Change life_satisfaction to only include countries in WHR
iso3_values <- unique(WHR_AS_comparison$COUNTRY_ISO3)
subset_life_satisfaction <- life_satisfaction[, iso3_values]

# Unidirectional change for age groups 1-5
is_increasing <- function(x) {
  all(diff(x) > 0)
}

# Function to check if a vector is strictly decreasing
is_decreasing <- function(x) {
  all(diff(x) < 0)
}

increasing_cols1 <- character(0)
decreasing_cols1 <- character(0)

# Loop through each column except "age_group"
for (col_name in colnames(subset_life_satisfaction)) {
  if (is_increasing(subset_life_satisfaction[[col_name]])) {
      increasing_cols1 <- c(increasing_cols1, col_name)
    }
  if (is_decreasing(subset_life_satisfaction[[col_name]])) {
      decreasing_cols1 <- c(decreasing_cols1, col_name)
  }
}

cat("Columns strictly increasing from age groups 1-5:", length(increasing_cols1)/length(iso3_values)*100, "\n")
cat("Columns strictly decreasing from age groups 1-5:", length(decreasing_cols1)/length(iso3_values)*100, "\n")


# Unidirectional change for age groups 1-4
subset_df <- subset_life_satisfaction[1:4, ]

increasing_cols2 <- character(0)
decreasing_cols2 <- character(0)

# Loop through each column except "age_group"
for (col_name in colnames(subset_df)) {
  if (col_name != "age_group") {
    # Check if the column is strictly increasing
    if (is_increasing(subset_df[[col_name]])) {
      increasing_cols2 <- c(increasing_cols2, col_name)
    }
    # Check if the column is strictly decreasing
    if (is_decreasing(subset_df[[col_name]])) {
      decreasing_cols2 <- c(decreasing_cols2, col_name)
    }
  }
}

cat("Columns strictly increasing from age groups 1-4:", length(increasing_cols2)/length(iso3_values)*100, "\n")
cat("Columns strictly decreasing from age groups 1-4:", length(decreasing_cols2)/length(iso3_values)*100, "\n")


# Change from age group 1 to age group 5
difference <- subset_life_satisfaction[5, ] - subset_life_satisfaction[1, ]
difference_vector <- as.vector(difference)
positive_count <- sum(difference_vector > 0, na.rm = TRUE)
negative_count <- sum(difference_vector < 0, na.rm = TRUE)

cat("Proportion of increases from age group 1 to 5", ":", positive_count/length(iso3_values)*100, "\n")
cat("Proportion of decreases from age group 1 to 5", ":", negative_count/length(iso3_values)*100, "\n")


# Change from age group 1 to age group 4
difference2 <- subset_life_satisfaction[4, ] - subset_life_satisfaction[1, ]
difference_vector2 <- as.vector(difference2)
positive_count2 <- sum(difference_vector2 > 0, na.rm = TRUE)
negative_count2 <- sum(difference_vector2 < 0, na.rm = TRUE)

cat("Proportion of increases from age group 1 to 5", ":", positive_count2/length(iso3_values)*100, "\n")
cat("Proportion of decreases from age group 1 to 5", ":", negative_count2/length(iso3_values)*100, "\n")
```

Find example where higher well-being at each age group is disguised by different age demographics. 
```{r}
subset_life_satisfaction <- subset_life_satisfaction[, -1]
rownames(subset_life_satisfaction) <- c("15-29", "30-44", "45-59", "60-74", "Over 74")

find_intersection <- function(input_country, life_satisfaction_data, whr_comparison_data) {
  # Function to compare countries in life satisfaction data
  compare_countries_all_groups <- function(input_country, data) {
    input_col <- which(colnames(data) == input_country)
    input_values <- data[, input_col]
    higher_countries <- character(0)
    for (i in 1:ncol(data)) {
      if (i == input_col)
        next
      if (all(data[, i] > input_values, na.rm = TRUE)) {
        higher_countries <- c(higher_countries, colnames(data)[i])
      }
    }
    return(higher_countries)
  }
  
  # Find countries higher in all age groups
  higher_in_all_groups <- compare_countries_all_groups(input_country, life_satisfaction_data)
  
  # Find rank of input country in WHR data
  rank <- whr_comparison_data[whr_comparison_data$COUNTRY_ISO3 == input_country, "WHR_rank"]
  
  # Find countries lower in WHR rank
  lower_in_WHR <- whr_comparison_data %>%
    filter(WHR_rank > rank) %>%
    dplyr::select(COUNTRY_ISO3)
  
  # Find intersection
  intersection_result <- intersect(higher_in_all_groups, lower_in_WHR$COUNTRY_ISO3)
  
  return(intersection_result)
}

find_intersection("BFA", subset_life_satisfaction, WHR_AS_comparison)
```

Summarizing Main Statistics
```{r}
average_rank_diff <- mean(abs(WHR_AS_comparison$rank_difference))
average_score_diff <- mean(abs(WHR_AS_comparison$score_difference))
count_rank_diff_geq_5 <- sum(abs(WHR_AS_comparison$rank_difference) >= 5)
```

Writing files from data frames
```{r}
# Write Standardized_contributions to csv
