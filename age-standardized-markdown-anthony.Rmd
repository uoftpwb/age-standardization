...
WHR_summary <- WHR_data_across_years %>% filter(year >= 2020, year <= 2022)%>% 
  ...
  mutate(WHR_rank = rank(-Life.Ladder),
...
WHR_AS_comparison <- merge(WHR_summary, age_standardized_ranking, by="COUNTRY_ISO3", all=FALSE) %>%
  mutate(rank_difference = WHR_rank - AS_rank,
         score_difference = Life.Ladder - AS_score)
```
...
AS_summary <- AS_data_across_years %>% filter(year >= 2020, year <= 2022) %>% 
  group_by(Country.name) %>% 
  summarize(AS.Life.ladder = weighted.mean(WeightedMeanLS, n),
            Log.GDP.per.capita = weighted.mean(Log.GDP.per.capita, n, na.rm = TRUE),
            Social.support = weighted.mean(Social.support, n, na.rm = TRUE),
            Healthy.life.expectancy.at.birth = weighted.mean(Healthy.life.expectancy.at.birth, n, na.rm = TRUE),
            Freedom.to.make.life.choices = weighted.mean(Freedom.to.make.life.choices, n, na.rm = TRUE),
            Generosity = weighted.mean(Generosity, n, na.rm = TRUE),
            Perceptions.of.corruption = weighted.mean(Perceptions.of.corruption, n, na.rm = TRUE)) %>% 
  mutate(WHR_rank = rank(-AS.Life.ladder),
         COUNTRY_ISO3 = ifelse(Country.name == "Kosovo", "XKX", countrycode(Country.name, 'country.name', 'iso3c')))
```
...
Standardized_contributions <- Standardized_WHR_summary %>% 
  mutate(Log.GDP.per.capita.cont = (Log.GDP.per.capita - min(Standardized_WHR_summary$Log.GDP.per.capita, na.rm = TRUE)) * coef(Standardized_regression)["Log.GDP.per.capita"],
         Social.support.cont = (Social.support - min(Standardized_WHR_summary$Social.support, na.rm = TRUE)) * coef(Standardized_regression)["Social.support"],
         Healthy.life.expectancy.at.birth.cont = (Healthy.life.expectancy.at.birth - min(Standardized_WHR_summary$Healthy.life.expectancy.at.birth, na.rm = TRUE)) * coef(Standardized_regression)["Healthy.life.expectancy.at.birth"],
         Freedom.to.make.life.choices.cont = (Freedom.to.make.life.choices - min(Standardized_WHR_summary$Freedom.to.make.life.choices, na.rm = TRUE)) * coef(WHR_regression)["Freedom.to.make.life.choices"],
         Generosity.cont = (Generosity - min(Standardized_WHR_summary$Generosity, na.rm = TRUE)) * coef(Standardized_regression)["Generosity"],
         Perceptions.of.corruption.cont = (Perceptions.of.corruption - max(Standardized_WHR_summary$Perceptions.of.corruption, na.rm = TRUE)) * coef(Standardized_regression)["Perceptions.of.corruption"])
```
...
age_specific_ranking_1  <- gallup_main %>% 
  filter(age_group == "15-29",
         YEAR_CALENDAR >= 2020, YEAR_CALENDAR <= 2022) %>% 
  drop_na(Weighted.LS) %>% 
  group_by(COUNTRY_ISO3, COUNTRYNEW) %>%
  summarize(AS.LS = sum(Weighted.LS)/sum(WGT))
colnames(age_specific_ranking_1) <- c("COUNTRY_ISO3", "Country.name", "AS.LS")
...
```{r}
# Create data frames for each Specific_regression_data with additional age_group column
data1 <- Specific_regression_data_1 %>%
  dplyr::select(c(1, 5:10, 16)) %>%
  mutate(age_group = 1)

data2 <- Specific_regression_data_2 %>%
  dplyr::select(c(1, 5:10, 16)) %>%
  mutate(age_group = 2)

data3 <- Specific_regression_data_3 %>%
  dplyr::select(c(1, 5:10, 16)) %>%
  mutate(age_group = 3)

data4 <- Specific_regression_data_4 %>%
  dplyr::select(c(1, 5:10, 16)) %>%
  mutate(age_group = 4)

data5 <- Specific_regression_data_5 %>%
  dplyr::select(c(1, 5:10, 16)) %>%
  mutate(age_group = 5)
...
big_regression_data <- big_regression_data %>%
...
var(WHR_AS_comparison$Life.Ladder)
var(WHR_AS_comparison$AS_score)

var.test(WHR_AS_comparison$Life.Ladder, WHR_AS_comparison$AS_score, alternative = "less")

Gini(WHR_AS_comparison$Life.Ladder, conf.level = 0.95)
...
notable_countries <- comparison_2013_2023 %>% 
  filter(Life.Ladder > Ladder & AS_score.x < AS_score.y |
         Life.Ladder < Ladder & AS_score.x > AS_score.y) %>% 
  dplyr::select(COUNTRY_ISO3 = COUNTRY_ISO3, Country = region, WHR_2023 = Life.Ladder, AS_2023 = AS_score.x, WHR_2013 = Ladder, AS_2013 = AS_score.y)
...
significant_changes_in_rank <- WHR_AS_comparison %>% 
  ...
  dplyr::select(COUNTRY_ISO3, region, Life.Ladder, AS_score, rank_difference, score_difference)
```

Analysis of Age-Specific Rankings - Age Group 1
```{r}
specific_contributions_1 <- specific_contributions_1 %>%  
  mutate(WHR_rank = rank(-Life.Ladder)) %>% 
  mutate(rank_difference = AS_rank - WHR_rank,
         score_difference = AS.LS - Life.Ladder)
            
rd1 <- mean(abs(specific_contributions_1$rank_difference))
sd1 <- mean(abs(specific_contributions_1$score_difference))

significant_changes_in_rank_1 <- specific_contributions_1 %>% 
  filter(abs(rank_difference) >= 10) %>% 
  dplyr::select(COUNTRY_ISO3, Country = Country.name.x, Life.Ladder, AS.LS, rank_difference, score_difference)
```

Analysis of Age-Specific Rankings - Age Group 2
```{r}
specific_contributions_2 <- specific_contributions_2 %>%  
  mutate(WHR_rank = rank(-Life.Ladder)) %>% 
  mutate(rank_difference = AS_rank - WHR_rank,
         score_difference = AS.LS - Life.Ladder)
            
rd2 <- mean(abs(specific_contributions_2$rank_difference))
sd2 <- mean(abs(specific_contributions_2$score_difference))

significant_changes_in_rank_2 <- specific_contributions_2 %>% 
  filter(abs(rank_difference) >= 10) %>% 
  dplyr::select(COUNTRY_ISO3, Country = Country.name.x, Life.Ladder, AS.LS, rank_difference, score_difference)
```
...
```{r}
specific_contributions_3 <- specific_contributions_3 %>%  
  mutate(WHR_rank = rank(-Life.Ladder)) %>% 
  mutate(rank_difference = AS_rank - WHR_rank,
         score_difference = AS.LS - Life.Ladder)
            
rd3 <- mean(abs(specific_contributions_3$rank_difference))
sd3 <- mean(abs(specific_contributions_3$score_difference))

significant_changes_in_rank_3 <- specific_contributions_3 %>% 
  filter(abs(rank_difference) >= 10) %>% 
  dplyr::select(COUNTRY_ISO3, Country = Country.name.x, Life.Ladder, AS.LS, rank_difference, score_difference)
```
...
```{r}
specific_contributions_4 <- specific_contributions_4 %>%  
  mutate(WHR_rank = rank(-Life.Ladder)) %>% 
  mutate(rank_difference = AS_rank - WHR_rank,
         score_difference = AS.LS - Life.Ladder)
            
rd4 <- mean(abs(specific_contributions_4$rank_difference))
sd4 <- mean(abs(specific_contributions_4$score_difference))

significant_changes_in_rank_4 <- specific_contributions_4 %>% 
  filter(abs(rank_difference) >= 10) %>% 
  dplyr::select(COUNTRY_ISO3, Country = Country.name.x, Life.Ladder, AS.LS, rank_difference, score_difference)
```
...
```{r}
specific_contributions_5 <- specific_contributions_5 %>%  
  mutate(WHR_rank = rank(-Life.Ladder)) %>% 
  mutate(rank_difference = AS_rank - WHR_rank,
         score_difference = AS.LS - Life.Ladder)
            
rd5 <- mean(abs(specific_contributions_5$rank_difference))
sd5 <- mean(abs(specific_contributions_5$score_difference))

significant_changes_in_rank_5 <- specific_contributions_5 %>% 
  filter(abs(rank_difference) >= 10) %>% 
  dplyr::select(COUNTRY_ISO3, Country = Country.name.x, Life.Ladder, AS.LS, rank_difference, score_difference)

# Creating table summarizing how much change is in each rank and score for each age group
...
ranking_comparisons <- WHR_AS_comparison %>% 
  dplyr::select(COUNTRY_ISO3, region, Life.Ladder, WHR_rank, AS_score, AS_rank)
...
# Create graphpic, red is Guatemala and blue is Serbia (two countries with biggest change in either direction)
