...
# Adding a new row named "Dystopia" with minimum values for each column (except for Perceptions.of.corruption, where we take the maximum because the coefficient is negative)
  add_row(
    ...
    Log.GDP.per.capita = min(WHR_data_fig$Log.GDP.per.capita, na.rm = TRUE),
    Social.support = min(WHR_data_fig$Social.support, na.rm = TRUE),
    Healthy.life.expectancy.at.birth = min(WHR_data_fig$Healthy.life.expectancy.at.birth, na.rm = TRUE),
    Freedom.to.make.life.choices = min(WHR_data_fig$Freedom.to.make.life.choices, na.rm = TRUE),
    Generosity = min(WHR_data_fig$Generosity, na.rm = TRUE),
    Perceptions.of.corruption = max(WHR_data_fig$Perceptions.of.corruption, na.rm = TRUE)
  )
```
