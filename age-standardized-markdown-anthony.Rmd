...
# YEAR_WAVE refers to the year of the wave of data collection. Occasionally, these two variables do not match because some data collection waves are completed fully within the calendar year. 
...
Joint hypothesis tests involve testing multiple hypotheses simultaneously to determine whether they are collectively supported by the data. We use the "across_age_model", which combines regressions for all age groups, to test the hypotheses that a predictor has a statistically significant difference in their relationship to life satisfaction when we compare different age groups. 
```{r}
...
a <- specific_contributions_list[[1]]
```
