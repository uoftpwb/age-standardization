...
# Age standardize for each year then calculate the weighted average based on n of each country-year. 
...
age_matrix <- gallup_main %>%
  ...
  #summarise(WeightedMeanLS = sum(Weighted.LS, na.rm=T)/sum(WGT, na.rm=T)) %>%
  summarise(WeightedMeanLS = mean(Weighted.LS, na.rm=T)) %>%
  pivot_wider(names_from = COUNTRY_ISO3, values_from = WeightedMeanLS) %>%
...
ranking_comparison_wide <- ranking_comparisons %>% 
  dplyr::select(COUNTRY_ISO3, COUNTRYNAME = region, WHR_rank, AS_rank, AG1_rank, AG5_rank)
ranking_comparison_wide <- ranking_comparison_wide[order(ranking_comparison_wide$WHR_rank), ]
b <- ranking_comparison_wide[,3:6]
b_WHR <- b[,1]
b$WHR2 <- b_WHR

colnames(b) <- c("WHR", "Age-Standardized", "Age Group 1", "Age Group 5", "WHR2")

head(b)

melted_df <- reshape2::melt(b, id.vars = "WHR2")

# Create a ggplot tile plot
ggplot(melted_df, aes(x = variable, y = WHR2, fill = value)) +
  geom_tile() +
  scale_fill_viridis(direction=-1, name = "Ranking") +  # Use viridis color scale
...
ggplot(df_long, aes(x = variable, y = WHR2, fill = factor(value))) +
  ...
  scale_fill_manual(
    values = c(
      viridis(137, direction = -1)[1:89],
      "red",
      viridis(137, direction = -1)[91:123],
      "blue",
      viridis(137, direction = -1)[125:255]
    )
...
Researching Ethiopia and Laos Changes
```{r}
#Ethiopia
# Filter the relevant years for ethiopia
ethiopia <- age_matrix %>% select("YEAR_CALENDAR", "age_group", "ETH") %>% 
  filter((YEAR_CALENDAR <= 2012 & YEAR_CALENDAR >= 2010)|(YEAR_CALENDAR <= 2022 & YEAR_CALENDAR >= 2020))

ethiopia$YEAR_CALENDAR <- as.numeric(ethiopia$YEAR_CALENDAR)
...
ethiopia <- ethiopia %>%
  ...
  summarise(Avg_ETH = mean(ETH, na.rm = TRUE)) %>%
  arrange(age_group, Year_Group)

ggplot(ethiopia, aes(x = age_group, y = Avg_ETH, fill = Year_Group)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7, color = "black") +
  labs(title = "Average LS by Age Group and Year Group in Ethiopia", x = "Age Group", y = "Average LS") +
  scale_fill_manual(values = c("2010-2012" = "lightblue", "2020-2022" = "salmon")) +  # Adjust colors as needed
  theme_minimal()

#Laos
laos <- age_matrix %>% select("YEAR_CALENDAR", "age_group", "LAO") %>% 
  filter((YEAR_CALENDAR <= 2012 & YEAR_CALENDAR >= 2010)|(YEAR_CALENDAR <= 2022 & YEAR_CALENDAR >= 2020))

laos$YEAR_CALENDAR <- as.numeric(laos$YEAR_CALENDAR)
...
laos <- laos %>%
  group_by(age_group, Year_Group) %>%
  summarise(Avg_LAO = mean(LAO, na.rm = TRUE)) %>%
  arrange(age_group, Year_Group)

ggplot(laos, aes(x = age_group, y = Avg_LAO, fill = Year_Group)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7, color = "black") +
  labs(title = "Average LS by Age Group and Year Group in Laos", x = "Age Group", y = "Average LS") +
  scale_fill_manual(values = c("2010-2012" = "lightblue", "2020-2022" = "salmon")) +  # Adjust colors as needed
  theme_minimal()
```
...
ethiopia_n <- age_n_matrix %>% select("YEAR_CALENDAR", "age_group", "ETH") %>% 
  filter((YEAR_CALENDAR <= 2012 & YEAR_CALENDAR >= 2010)|(YEAR_CALENDAR <= 2022 & YEAR_CALENDAR >= 2020))
...
ethiopia = merge(ethiopia, ethiopia_n, by=c("YEAR_CALENDAR", "age_group"))
...
ethiopia <- ethiopia %>%
  group_by(age_group, Year_Group) %>%
  summarise(Avg_LS = weighted.mean(ETH.x, w = ETH.y, na.rm = TRUE)) %>%
  arrange(Year_Group, age_group)

ggplot(ethiopia, aes(x = age_group, y = Avg_LS, fill = Year_Group)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7, color = "black") +
  labs(title = "Average LS by Age Group and Year Group in Ethiopia", x = "Age Group", y = "Average LS") +
  scale_fill_manual(values = c("2010-2012" = "lightblue", "2020-2022" = "salmon")) +  # Adjust colors as needed
  theme_minimal()
...
laos_n <- age_n_matrix %>% select("YEAR_CALENDAR", "age_group", "LAO") %>% 
  filter((YEAR_CALENDAR <= 2012 & YEAR_CALENDAR >= 2010)|(YEAR_CALENDAR <= 2022 & YEAR_CALENDAR >= 2020))

laos = merge(laos, laos_n, by=c("YEAR_CALENDAR", "age_group"))
...
laos <- laos %>%
  group_by(age_group, Year_Group) %>%
  summarise(Avg_LS = weighted.mean(LAO.x, w = LAO.y, na.rm = TRUE)) %>%
  arrange(Year_Group, age_group)

ggplot(laos, aes(x = age_group, y = Avg_LS, fill = Year_Group)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7, color = "black") +
  labs(title = "Average LS by Age Group and Year Group in Laos", x = "Age Group", y = "Average LS") +
  scale_fill_manual(values = c("2010-2012" = "lightblue", "2020-2022" = "salmon")) +  # Adjust colors as needed
  theme_minimal()
...
Ethiopia has decreasing WHR and increasing AS. 

2013: Increasing LS as age goes up, but there are few old people.
2023: Inverse U-shape, more old people. 

In both years, majority are young, so decrease in happiness of young people lead to decrease in WHR.
However, When you standardize such that the weight of young people is smaller,
the increase in middle age people is weighed more heavily and thus AS increases. 



Laos has increasing WHR and decreasing AS. 

2013: U-shape with high LS in youngest.
2023: Decreasing steadily but more mild.

WHR increases as Laos has seen LS increase in 4 of 5 age groups. 
However, the drop in the LS of the youngest group is large, so when standardized,
