...
WHR_summary <- WHR_data %>% filter(year >= 2020, year <= 2022)%>% 
  ...
  summarize(Life.Ladder = weighted.mean(Life.Ladder,nna.rm = TRUE),
...
```{r}
gallup_main <- readRDS(
  "/Users/makototakahara/Downloads/(231027)GWP_cleaned_ageStandardizedWellbeing.rds")
gallup_main$Weighted.LS <- as.numeric(gallup_main$WGT*gallup_main$WP16)

gallup_main <- gallup_main %>% 
  filter(!is.na(WP1220),
         YEAR_CALENDAR %in% c(2020, 2021, 2022)) %>% 
...
Calculating Contributions of Predictors
